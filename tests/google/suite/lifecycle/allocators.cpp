...
TEST(AllocatorPropagation, CopyConstructionSelectsSourceAllocatorTag) {
    ...
    EXPECT_GT(AllocStats::allocations[5], 0);
    ...
    EXPECT_GT(AllocStats::allocations[5], allocationsUnderTag5Before);
    EXPECT_EQ(b.size(), 2u);
}
...
TEST(AllocatorPropagation, CopyAssignmentDoesNotPropagateAllocator) {
    ...
    EXPECT_GT(AllocStats::allocations[1], allocationsUnderTag1Before);
...
TEST(AllocatorPropagation, MoveAssignmentEqualAllocatorsStealsBuffer) {
    ...
    EXPECT_EQ(AllocStats::allocations[7], allocationsBefore);
...
TEST(AllocatorPropagation, MoveAssignmentUnequalAllocatorsUsesTargetAllocator) {
    ...
    EXPECT_GT(AllocStats::allocations[10], allocationsUnderTargetTagBefore);
    EXPECT_EQ(target.size(), 5u);
    EXPECT_EQ(target[0], 0);
}
...
// allocator's tag.
TEST(AllocatorPropagation, DestructorDeallocatesUnderOwnAllocator) {
    reset_alloc_stats();
    ...
    EXPECT_GT(AllocStats::deallocations[3], 0);
}
