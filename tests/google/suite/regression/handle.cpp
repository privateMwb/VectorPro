...
TEST(ListenerHandleRegression, UnsubscribeEarlierHandleStrandsLaterHandle) {
    ...
    v.push_back(1);
    EXPECT_EQ(count0, 0); // correctly removed
    EXPECT_EQ(count1, 1); // shifted to index 0, still fires
    EXPECT_EQ(count2, 1); // shifted to index 1, still fires
    ...
    EXPECT_EQ(count2, 2); // still fires despite the "unsubscribe" call above
}
...
TEST(ListenerHandleRegression, UnsubscribeShiftedHandleRemovesWrongListener) {
    ...
    EXPECT_EQ(count0, 0); // correctly removed at the start
    EXPECT_EQ(count1, 1); // h1's own callback is untouched, now at index 0
    EXPECT_EQ(count2, 0); // removed under a handle that was never issued for it
}
