...
// fails the build even if every value-level CHK below happens to pass.
...
static void matches_reference_model_over_five_thousand_steps() {
    ...
    for (int step = 0; step < 5000; ++step) {
        ...
        CHK(actual.size() == reference.size());
    }

    CHK(actual.size() == reference.size());
    for (std::size_t i = 0; i < reference.size(); ++i) {
        CHK(actual[i] == reference[i]);
    }
}
...
static void move_only_element_survives_push_back_and_move() {
    ...
    CHK(moved.size() == 50);
    for (int i = 0; i < 50; ++i) {
        CHK(moved[static_cast<std::size_t>(i)] != nullptr);
        CHK(*moved[static_cast<std::size_t>(i)] == i);
    }

    Vector<std::unique_ptr<int>> assigned;
    assigned = std::move(moved);
    CHK(moved.empty());
    CHK(assigned.size() == 50);
    CHK(*assigned[0] == 0);
    CHK(*assigned[49] == 49);
}
