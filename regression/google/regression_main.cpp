...
int main(int argc, char* argv[]) {
    ...
    try {
        if (argc == 1) {
            ...
            baselineFile = "benchmarks/baselines/" + std::string(argv[1]) + "/g" + std::string(argv[1]) + ".json";
            currentFile = "benchmarks/results/google_benchmark_results.json";
        } else if (argc == 3) {
            baselineFile = "benchmarks/baselines/" + std::string(argv[1]) + "/g" + std::string(argv[1]) + ".json";
            currentFile = "benchmarks/baselines/" + std::string(argv[2]) + "/g" + std::string(argv[2]) + ".json";
        } else {
            std::cerr << "Usage:\n"
                      << "  google regressions\n"
