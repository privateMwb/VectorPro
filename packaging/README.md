...
`packaging/recipes/vectorpro/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `VectorPro-<version>`, not
  `VectorPro-<version>`** — the GitHub repo is `VectorPro`; the CMake
  project/library target inside it is `VectorPro`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/VectorPro -name "*.cmake"
```

Expect `VectorProConfig.cmake`, `VectorProConfigVersion.cmake`,
`VectorProTargets.cmake` (plus a per-config
`VectorProTargets-<config>.cmake`) under `lib/cmake/VectorPro/`.
...
target_link_libraries(test PRIVATE VectorPro::VectorPro)
EOF
...
just default-constructs a `VectorPro::VectorPro` and returns, so success
...
```bash
conan create packaging/recipes/vectorpro/all --version 1.0.0 \
...
landing in the `package()` step's output, and — since VectorPro is
header-only, no compiled artifact — a single shared `package_id`
regardless of settings, not one computed per settings combination.

This step also automatically builds and runs
`packaging/recipes/vectorpro/all/test_package/` against the freshly-built
package — a `VectorPro linked and constructed successfully.` line in
